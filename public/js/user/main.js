  ...
  document.addEventListener("DOMContentLoaded", function () {
        ...
        mainTabTriggers.forEach((trigger) => {
          trigger.addEventListener("click", function () {
            ...
            const sectionSelectorRow = this.closest(
              ".glnc-section-selector-row",
            );
        ...
        const moduleHeaders = document.querySelectorAll(
          ".glnc-js-module-toggle",
        );
        moduleHeaders.forEach((header) => {
          header.addEventListener("click", function () {
            const currentWrapper = this.closest(
            ...
            const isCollapsed =
              currentWrapper.classList.contains("state-collapsed");
        ...
        const lessonHeaders = document.querySelectorAll(
          ".glnc-js-lesson-toggle",
        );
        lessonHeaders.forEach((lessonHeader) => {
          lessonHeader.addEventListener("click", function () {
            const currentLessonWrapper = this.closest(
    ...
    //   request management user profile
     document.querySelectorAll('.provider-item').forEach(item => {
            item.addEventListener('click', function() {
                document.querySelectorAll('.provider-item').forEach(i => i.classList.remove('selected'));
...
document.addEventListener("DOMContentLoaded", function() {
 ...
 document.addEventListener("DOMContentLoaded", function () {
            ...
            const visibilityTriggers = document.querySelectorAll('.visibility-btn-trigger');
        ...
        function switchTabPanel(panelId, buttonElement) {
            document.querySelectorAll('.glnc-tab-pill').forEach(btn => btn.classList.remove('active'));
        ...
        // Advanced Analytics
         ...
         function toggleCustomDropdownMenu(trigger) {
            ...
            document.querySelectorAll('.gl-dropdown-wrapper').forEach(w => { if (w !== currentWrapper) w.classList.remove('open'); });
            currentWrapper.classList.toggle('open');
        }

        function filterDropdownListTrack(input) {
            const keyword = input.value.toLowerCase();
            const rows = input.parentElement.parentElement.querySelectorAll('.gl-menu-item-row');
            rows.forEach(r => {
                const text = r.querySelector('.gl-menu-item-text').innerText.toLowerCase();
                r.style.setProperty("display", text.includes(keyword) ? "flex" : "none", "important");
            });
        }
        ...
        function handleOptionSelectionClick(row) {
            ...
            const lbl = row.parentElement.parentElement.parentElement.querySelector('.trigger-label-render');

            checkbox.checked = !checkbox.checked;
            if (checkbox.checked) { row.classList.add('selected'); lbl.innerText = txt; }
        ...
        $(function () {
            $('#dashboardDateRangeSelector').daterangepicker({
                ...
                $('#dashboardDateRangeSelector .date-display-target').text(start.format('MMM D, YYYY') + ' - ' + end.format('MMM D, YYYY'));
            });
        });

        document.addEventListener('click', function (e) {
            if (!e.target.closest('.gl-dropdown-wrapper')) {
                document.querySelectorAll('.gl-dropdown-wrapper').forEach(w => w.classList.remove('open'));
            }
        });
